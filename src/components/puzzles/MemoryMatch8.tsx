...
export default function MemoryMatch8({ onComplete }: Props) {
        ...
        {cards.map(card => {
          ...
          return (
            <div key={card.id} className={`memory-card ${isFlipped ? 'flipped' : ''}`}
              onClick={() => handleFlip(card.id)} style={{ height: '65px' }}>
              <div className="memory-card-face memory-card-back">❓</div>
              <div className="memory-card-face memory-card-front">{card.matched ? '✅' : card.symbol}</div>
            </div>
          );
        })}
      </div>
