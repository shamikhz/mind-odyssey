...
export default function TowerOfHanoi({ onComplete }: Props) {
  ...
  const handlePegClick = (pegIdx: number) => {
    if (selectedPeg === null) {
      ...
      if (newPegs[2].length === 3) setTimeout(onComplete, 500);
    }
  };

  return (
    <div style={{ width: '100%', textAlign: 'center' }}>
      <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginBottom: '16px' }}>Moves: {moves} (Minimum: 7)</p>
      <div className="hanoi-container">
        {pegs.map((peg, pi) => (
          <div key={pi} className="hanoi-peg" onClick={() => handlePegClick(pi)}
            style={{ cursor: 'pointer', border: selectedPeg === pi ? '2px solid var(--accent-primary)' : '2px solid transparent', borderRadius: 'var(--radius-md)', padding: '8px' }}>
            {peg.map((disk, di) => (
              <div key={di} className="hanoi-disk" style={{
                width: `${disk * 30 + 20}px`,
