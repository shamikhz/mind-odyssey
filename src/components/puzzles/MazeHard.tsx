...
function generateMaze(size: number): number[][] {
  ...
  carve(1, 1); maze[1][0] = 0; maze[size-2][size-1] = 0;
  return maze;
}

export default function MazeHard({ onComplete }: Props) {
  const size = 15;
    ...
    <div style={{ width: '100%', maxWidth: '400px', textAlign: 'center' }}>
      <div className="maze-grid" style={{ gridTemplateColumns: `repeat(${size}, 1fr)`, marginBottom: '12px' }}>
        {maze.map((row, r) => row.map((cell, c) => {
          ...
          const isV = visited.has(`${r},${c}`);
          return <div key={`${r}-${c}`} className={`maze-cell ${cell===1?'maze-wall':isPlayer?'maze-player':isEnd?'maze-end':isV?'maze-visited':'maze-path'}`} />;
        }))}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 36px)', gap: '3px', justifyContent: 'center' }}>
        <div /><button className="btn btn-sm btn-secondary" onClick={() => move(-1,0)}>↑</button><div />
        <button className="btn btn-sm btn-secondary" onClick={() => move(0,-1)}>←</button><div />
